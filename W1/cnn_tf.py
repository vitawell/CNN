...
def create_placeholders(n_H0, n_W0, n_C0, n_y):
    ...
    因为我们使用的是小批量数据块，输入的样本数量可能不固定，
        所以我们在数量那里我们要使用None作为可变数量。
    """
...
def compute_cost(Z3, Y):
    ...
    tf.nn.softmax_cross_entropy_with_logits()：这个函数既计算softmax的激活，也计算其损失
...
def model(X_train, Y_train, X_test, Y_test, learning_rate=0.009,
          num_epochs=150, minibatch_size=64, print_cost=True, isPlot=True):
    """
...
_, _, parameters = model(X_train, Y_train, X_test, Y_test, learning_rate=0.005, num_epochs=600)
# learning_rate=0.005, num_epochs=600（大概需要跑30分钟）
# 训练集准确度：1.0
# 测试集准确度：0.89166665
